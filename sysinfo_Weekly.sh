...
# Email 
SUBJECT="FW DEV SERVERS DISK USAGE WEEKLY REPORT"
EMAIL="mstertz@micron.com smalkani@micron.com"
FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT="/fwdev_servers_diskusage_weekly_report.txt"
VAR=$(</Script_Weekly/sysinfo_dependent.sh)
# create new file
>$FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT
# connect each host and pull up user listing
echo "                                                                            *****FW DEVELOPMENT SERVERS DISK USAGE REPORT*****                                              " >>$FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT            
echo -e "                                                                                 ****************************************                                                   \n" >>$FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT 
for host in $SERVERS
do
echo "=============================" >>$FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT
echo " -> HOST: $host " >>$FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT
echo -e "=============================\n" >>$FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT
ssh $host $VAR >> $FWDEV_SERVERS_DISKUSAGE_WEEKLY_REPORT
done
# send an email using /bin/mail
/bin/mail -a "/fwdev_servers_diskusage_weekly_report.txt" -s "$SUBJECT" "$EMAIL" < /Script_Weekly/Mess_body.sh
